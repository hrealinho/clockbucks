...
class MeetingRepository(BaseRepository[DBMeeting, MeetingCreate, MeetingUpdate]):
    ...
    def get(self, id: UUID) -> Optional[DBMeeting]:
        ...
        return self.db.query(self.model).options(
            joinedload(self.model.participants).joinedload(DBMeetingParticipant.participant)
    ...
    def create(self, obj_in: MeetingCreate) -> DBMeeting:
        ...
        for participant_data in obj_in.participants:
            ...
            db_participant = self.db.query(DBParticipant).filter(
                DBParticipant.id == participant_data.id
            ).first()
    ...
    def get_meeting_statistics(self) -> Dict[str, Any]:
        ...
        stats = self.db.query(
            ...
            func.sum(case([(self.model.status == 'completed', 1)], else_=0)).label('completed_meetings'),
            func.sum(case([(self.model.status == 'in_progress', 1)], else_=0)).label('active_meetings'),
            func.sum(case([(self.model.status == 'scheduled', 1)], else_=0)).label('scheduled_meetings')
        ).first()
    ...
    def end_meeting(self, meeting_id: UUID, total_cost: float = None) -> Optional[DBMeeting]:
            ...
            if total_cost is not None:
                ...
                if db_meeting.duration_minutes and db_meeting.participants:
                    for meeting_participant in db_meeting.participants:
                        individual_cost = (meeting_participant.hourly_rate_at_time / 60) * db_meeting.duration_minutes
