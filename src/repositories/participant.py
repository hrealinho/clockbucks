...
class ParticipantRepository(BaseRepository[DBParticipant, ParticipantCreate, ParticipantUpdate]):
    """Repository for participant data access."""
    
    def get(self, id: UUID) -> Optional[DBParticipant]:
        """Get participant by ID."""
        try:
            return self.db.query(DBParticipant).filter(
                and_(DBParticipant.id == id, DBParticipant.is_active == True)
            ).first()
        ...
        try:
            ...
            if filters:
                if "department" in filters:
                    query = query.filter(DBParticipant.department == filters["department"])
                if "role" in filters:
                    query = query.filter(DBParticipant.role == filters["role"])
                if "min_rate" in filters:
                    query = query.filter(DBParticipant.hourly_rate >= filters["min_rate"])
                if "max_rate" in filters:
                    query = query.filter(DBParticipant.hourly_rate <= filters["max_rate"])
    ...
    def create(self, obj_in: ParticipantCreate) -> DBParticipant:
        ...
        try:
            ...
            if obj_in.email and self.get_by_email(obj_in.email):
                raise DuplicateError(f"Participant with email {obj_in.email} already exists")
